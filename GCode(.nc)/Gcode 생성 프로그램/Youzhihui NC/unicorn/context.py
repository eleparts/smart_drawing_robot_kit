...
class GCodeContext:
    def __init__(self, xy_feedrate,penup_feedrate, z_feedrate, start_delay, stop_delay, pen_up_angle, pen_down_angle, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      self.preamble = [
        ...
        #"G21 (metric ftw)",
        #"G90 (absolute mode)",
		#"G00 Z%0.2F (pen up)" % self.pen_up_angle,
		"G0Z20.000",
		"G0X0.000Y0.000",
      ...
      self.postscript = [
        "",
				"(end of print job)",
				"G00 X0 Y0 Z%0.2F (pen up)" % self.pen_up_angle,
				...
				#"G1 X%0.2F Y%0.2F (go home)" % (self.x_home, self.y_home),
				#"M18 (drives off)",
      ]

      self.registration = [
        #"G00 Z%d (pen down)" % (self.pen_down_angle),
        #"G4 P%d (wait %dms)" % (self.start_delay, self.start_delay),
        #"G00 Z%d (pen up)" % (self.pen_up_angle),
        #"G4 P%d (wait %dms)" % (self.stop_delay, self.stop_delay),
      ...
      self.sheet_footer = [
        ...
        #"G90 (absolute mode)",
        #"G0 X%0.2f Y%0.2f" % (self.x_home, self.y_home),
    ...
    def go_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing: 
            ...
            #self.codes.append("G4 P%d (wait %dms)" % (self.stop_delay, self.stop_delay))
            self.drawing = False
        self.codes.append("G01 X%0.2f Y%.2f" % (x-self.x_home,self.y_home-y))
      self.last = (x,y)
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            self.codes.append("G00 Z%0.2F (pen down)" % self.pen_up_angle)
            #self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
        self.codes.append("G01 X%0.2f Y%0.2f" % (x-self.x_home,self.y_home-y))
      self.last = (x,y)
